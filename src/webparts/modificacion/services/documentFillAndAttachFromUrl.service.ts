...
export async function fillAndAttachFromServerRelativeUrl(params: {
  context: WebPartContext;
  webUrl: string;
  listTitle: string;
  itemId: number;
  originalFileName: string;
  sourceFileServerRelativeUrl: string;
  titulo: string;
  instanciaRaw: string;
  ...
  if (!params.sourceFileServerRelativeUrl) {
    return { ok: false, error: `No se recibió la URL origen de "${cleanName}".` };
  }
  ...
  const fileResponse = await fetch(ensureAbsoluteUrl(params.webUrl, params.sourceFileServerRelativeUrl), {
