...
import { addAttachment, addListItem, deleteListItem, getAllItems, getAttachmentFiles, recycleFile, spGetJson, spPostJson, updateListItem } from './sharepointRest.service';

type LogFn = (s: string) => void;

const PROCESOS_ROOT = '/sites/SistemadeGestionDocumental/Procesos';

export interface IFase6RollbackAttachment {
  fileName: string;
  content: Blob;
}

export interface IFase6RollbackDiagramEntry {
  originalId: number;
  payload: any;
  attachments: IFase6RollbackAttachment[];
}

export interface IFase6RollbackEntry {
  solicitudId: number;
  ...
  sourceType?: 'direct_baja' | 'parent_regenerated';
...
async function buildProcesosRestorePayload(context: WebPartContext, webUrl: string, oldMetadata: any): Promise<any> {
  ...
  const payload: any = {
    ...
    Descripcion: oldMetadata?.Descripcion || '',
...
async function getDiagramasItemSnapshot(
  context: WebPartContext,
  webUrl: string,
  itemId: number
): Promise<IFase6RollbackDiagramEntry | null> {
  try {
    ...
    const item = await spGetJson<any>(
      context,
      `${webUrl}/_api/web/lists/getbytitle('Diagramas de Flujo')/items(${itemId})?$select=Id,Title,Codigo,${solicitudFieldId}`
    );
    const payload: any = {
      Title: item?.Title || '',
      Codigo: item?.Codigo || ''
    ...
    if (solicitudIds.length) {
      payload[solicitudFieldId] = solicitudIsMulti ? solicitudIds : solicitudIds[0];
    }

    const attachmentsInfo = await getAttachmentFiles(context, webUrl, 'Diagramas de Flujo', itemId);
    const attachments: IFase6RollbackAttachment[] = [];
    for (let i = 0; i < attachmentsInfo.length; i++) {
      const file = attachmentsInfo[i];
      const response = await fetch(`${new URL(webUrl).origin}${file.ServerRelativeUrl}`, {
...
async function restoreDiagramas(
  context: WebPartContext,
  webUrl: string,
  diagrams: IFase6RollbackDiagramEntry[],
  log: LogFn
): Promise<void> {
  for (let i = 0; i < diagrams.length; i++) {
    const diagram = diagrams[i];
    const newId = await addListItem(context, webUrl, 'Diagramas de Flujo', diagram.payload || {});
    for (let j = 0; j < (diagram.attachments || []).length; j++) {
      const attachment = diagram.attachments[j];
      await addAttachment(context, webUrl, 'Diagramas de Flujo', newId, attachment.fileName, attachment.content);
    }
    log(`🧭 Fase 6 rollback | Diagrama restaurado: ${diagram.originalId} -> ${newId}`);
  }
}
...
async function restoreRelacionesDocumentosPadre(params: {
  ...
  params.log(`🔗 Fase 6 rollback | Relaciones restauradas al padre ${params.oldParentSolicitudId}`);
}
...
async function restoreChildSolicitudesDocPadres(params: {
  ...
  params.log(`👨‍👧 Fase 6 rollback | DocPadres restaurados al padre ${params.oldParentSolicitudId}`);
}
...
async function restoreChildProcessParentReferences(params: {
  ...
  params.log(`👨‍👧 Fase 6 rollback | Referencias en Procesos restauradas al padre vigente anterior`);
}

export async function snapshotDiagramasForRollback(params: {
  context: WebPartContext;
  webUrl: string;
  diagramIds: number[];
}): Promise<IFase6RollbackDiagramEntry[]> {
  const ids = Array.from(new Set((params.diagramIds || []).filter((id) => Number(id) > 0)));
  const snapshots: IFase6RollbackDiagramEntry[] = [];
  for (let i = 0; i < ids.length; i++) {
    const snapshot = await getDiagramasItemSnapshot(params.context, params.webUrl, ids[i]);
    if (snapshot) {
...
export async function rollbackModificacionFase6(params: {
  context: WebPartContext;
  webUrl: string;
  entries: IFase6RollbackEntry[];
  diagramEntries?: IFase6RollbackDiagramEntry[];
  log?: LogFn;
}): Promise<void> {
  const log = params.log || (() => undefined);
  const entries = Array.isArray(params.entries) ? params.entries : [];
  const diagramEntries = Array.isArray(params.diagramEntries) ? params.diagramEntries : [];

  for (let i = entries.length - 1; i >= 0; i--) {
    const entry = entries[i];

    try {
      if (entry.sourceType === 'parent_regenerated') {
        if (entry.historicoUrl && entry.oldOriginalUrl) {
          ...
          log(`♻️ Fase 6 rollback | Padre restaurado desde Históricos: ${entry.oldOriginalUrl}`);
          ...
          if (entry.oldOriginalMetadata) {
            ...
            log(`🧾 Fase 6 rollback | Metadata del padre restaurada: ${entry.oldOriginalUrl}`);
          }
        }
        ...
        if (entry.replacementPublishedUrl) {
          ...
          log(`🗑️ Fase 6 rollback | Publicación nueva reciclada: ${entry.replacementPublishedUrl}`);
        }

        if (entry.tempFileUrl) {
          await recycleFile(params.context, params.webUrl, entry.tempFileUrl);
          log(`🗑️ Fase 6 rollback | TEMP reciclado: ${entry.tempFileUrl}`);
        }
        ...
        if (entry.replacementSolicitudId) {
          try {
            ...
            log(`🗑️ Fase 6 rollback | Nueva solicitud padre eliminada: ${entry.replacementSolicitudId}`);
          } catch (_deleteError) {
      ...
      } else if (entry.historicoUrl && entry.oldOriginalUrl) {
        await moveFileByPath(params.context, params.webUrl, entry.historicoUrl, entry.oldOriginalUrl, true);
        log(`♻️ Fase 6 rollback | Documento restaurado desde Históricos: ${entry.oldOriginalUrl}`);
        ...
        if (entry.oldOriginalMetadata) {
          ...
          log(`🧾 Fase 6 rollback | Metadata restaurada: ${entry.oldOriginalUrl}`);
        }
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      log(`⚠️ Fase 6 rollback | No se pudo restaurar el documento ${entry.oldOriginalUrl} -> ${message}`);
    }
    ...
    try {
      ...
      log(`♻️ Fase 6 rollback | Solicitud restaurada: ${entry.solicitudId}`);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      log(`⚠️ Fase 6 rollback | No se pudo restaurar la solicitud ${entry.solicitudId} -> ${message}`);
    }
  }

  try {
    await restoreDiagramas(params.context, params.webUrl, diagramEntries, log);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    log(`⚠️ Fase 6 rollback | No se pudieron restaurar los diagramas -> ${message}`);
  }
}
